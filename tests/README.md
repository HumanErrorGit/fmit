...
Note: `jom -f Makefile.Release` does **not** work in this environment -- on the
Makefiles generated by Qt 6.8.3's qmake here it fails with a false "dependent
does not exist" error. Use `nmake -f Makefile.Release` instead; that is what
was actually used to build and run this harness (15/15 checks passed).
