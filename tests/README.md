...
Note: `jom -f Makefile.Release` does **not** work on this out-of-source layout --
qmake's extra ".." level in moc dependency paths (from running `qmake
dsp_test.pro` outside the repo's build directory) trips a false "dependent
does not exist" error in jom, which `nmake` tolerates on the same Makefile.
Use `nmake -f Makefile.Release` instead; that is what was actually used to
build and run this harness (15/15 checks passed). This is specific to
shadow builds, not this machine: CI's Windows job runs `jom` successfully
in-source at the repo root.
